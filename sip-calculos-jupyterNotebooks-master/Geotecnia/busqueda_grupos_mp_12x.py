...
def main():
    ...
    pocho = data_services.recuperar_mejor_grupo_micropilotes_12x(torres_cubiertas,grupos_seleccionados,30)
    while pocho:
        print(pocho["grupo"], "\t", pocho["cobertura"],"\t",pocho["tipo"],"\t",pocho["resistencia_min_conc"],"\t",pocho["proc_iny"],"\t",pocho["barra_nombre"])
        data_services.guardar_mp_grupo(pocho, "IGU-12x")
        torres_cubiertas += pocho["torres_cubiertas"]
        if pocho["cobertura"] < 2:
            pocho = data_services.recuperar_mejor_grupo_micropilotes_12x(torres_cubiertas,grupos_seleccionados,1000)
        elif pocho["cobertura"] < 5:
            pocho = data_services.recuperar_mejor_grupo_micropilotes_12x(torres_cubiertas,grupos_seleccionados,60)
        else:
            pocho = data_services.recuperar_mejor_grupo_micropilotes_12x(torres_cubiertas,grupos_seleccionados,30)
