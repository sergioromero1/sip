...
class EvaluadorParrillas():

    def evaluar(self, parrilla: Parrilla, torre: Torre, perfil: Perfil, info_cargas: Cargas, FSC: float, FST_g: {float}, FST_c: {float}, k: {int}, t: {int}, S_max_adm_c: {float}, S_max_adm_g: {float}, FSV: {float}, FSL: {float}):
        """Realiza todas las evaluaciones de estabilidad y demás restrcciones en la zapata
                
        Arguments:
            parrilla {Parrrilla} -- Parrilla a evaluar
        ...
        evaluacion["cap_port-comp_max"] = self.evaluar_capacidad_portante(parrilla, cargas, FSC)
        # Capacidad portante - máxima longitudinal
        cargas = info_cargas.obtener_cargas_trabajo_maxima_longitudinal_cartesiano(torre)
        evaluacion["cap_port-long_max"] = self.evaluar_capacidad_portante(parrilla, cargas, FSC)
        # Capacidad portante - máxima transversal
        cargas = info_cargas.obtener_cargas_trabajo_maxima_transversal_cartesiano(torre)
        evaluacion["cap_port-tran_max"] = self.evaluar_capacidad_portante(parrilla, cargas, FSC)
        ...
        evaluacion["arrancamiento-tens_max"] = self.evaluar_arrancamiento(parrilla, perfil, cargas, q_ult, FSC, FST_g, FST_c)
        
        # Asentamiento - máxima compresión
        cargas = info_cargas.obtener_cargas_trabajo_maxima_compresion_cartesiano(torre)
        cargas_eds = info_cargas.obtener_cargas_trabajo_maxima_compresion_eds_cartesiano(torre)
        evaluacion["asentamiento-comp_max"] = self.evaluar_asentamiento(parrilla, cargas, cargas_eds, k, t, S_max_adm_c, S_max_adm_g)
        ...
        evaluacion["volcamiento-tens_max"] = self.evaluar_volcamiento(parrilla, (F_zc, max(F_xc, F_yc)), T_u, FSV)
        # Volcamiento - máxima longitudinal
        F_zc, F_xc, F_yc = info_cargas.obtener_cargas_trabajo_maxima_longitudinal_cartesiano(torre)
        evaluacion["volcamiento-long_max"] = self.evaluar_volcamiento(parrilla, (F_zc, F_xc), T_u, FSV)
        # Volcamiento - máxima transversal
        F_zc, F_xc, F_yc = info_cargas.obtener_cargas_trabajo_maxima_transversal_cartesiano(torre)
        evaluacion["volcamiento-tran_max"] = self.evaluar_volcamiento(parrilla, (F_zc, F_yc), T_u, FSV)
        
        # Deslizamiento - máxima longitudinal
        F_zc, F_xc, F_yc = info_cargas.obtener_cargas_trabajo_maxima_longitudinal_cartesiano(torre)
        evaluacion["deslizamiento-long_max"] = self.evaluar_deslizamiento(parrilla, (F_zc, F_xc), FSL)
        # Deslizamiento - máxima transversal
        F_zc, F_xc, F_yc = info_cargas.obtener_cargas_trabajo_maxima_transversal_cartesiano(torre)
        evaluacion["deslizamiento-tran_max"] = self.evaluar_deslizamiento(parrilla, (F_zc, F_yc), FSL)

        return evaluacion

    def evaluar_capacidad_portante(self, parrilla: Parrilla, cargas: List[float], FSC: float) -> Tuple[float,bool,float,float,bool]:
        """
        Evalua la zapata respecto a la compresión
        
        Arguments:
            parrilla {Parrilla} -- Parrilla a evaluar
        ...
        # Capacidad
        q_ult, memoria_q_ult = parrilla.calculo_capacidad_portante(T, F_zc)

        # Solicitud
        #Q_max, Q_min, memoria_Q = parrilla.calculo_esfuerzo_actuante_sobre_suelo(F_xc, F_yc, F_zc)

        Q_max = parrilla.esf_act_ingedisa + (parrilla.peso + parrilla.peso_relleno()) / parrilla.B**2
        ...
        fs_Q_max = round(q_ult / Q_max, 2)
        fs_Q_max = self.ajustar_fs(fs_Q_max, FSC, 0.01)
    ...
    def evaluar_arrancamiento(self, parrilla: Parrilla, perfil: Perfil, cargas: List[float], q_ult: float, FSC: float, FST_g: float, FST_c: float) -> Tuple[float,float,bool]:
        """
        ...
        tipo_mat_predominante = perfil.calcular_tipo_mat_predominante(0, parrilla.D)
        ...
        # Capacidad
        q_adm = q_ult / FSC
        T_u, memoria = parrilla.calculo_tension(q_adm)
    ...
    def evaluar_asentamiento(self, parrilla: Parrilla, cargas: List[float], cargas_eds: List[float], k: int, t: int, S_max_adm_c: float, S_max_adm_g: float) -> Tuple[float,float,bool]:
        """
        Evalua la parrilla respecto a la tensión (arrancamiento)
        ...
        tipos_mat_porcen = parrilla.perfil.calcular_porcentaje_tipo_mat(parrilla.D, parrilla.D + 2 * parrilla.B)
        ...
        # Capacidad
        S_e, S_c, memoria_e, memoria_c = parrilla.calculo_asentamiento(k, F_zc, F_zc_eds, t)
        S_e = round(S_e, 6)
        S_c = round(S_c, 6)
        S = round(S_e + S_c, 6)
    ...
    def evaluar_volcamiento(self, parrilla: Parrilla, cargas: List[float], T_u: float, FSV: float) -> Tuple[float,float,bool]:
        """
        ...
        Mv, Me, memoria = parrilla.calculo_volcamiento(F_zc, F, T_u)

        Mv = parrilla.mv_ingedisa

        # Factor de seguridad
        fs = round(Me / Mv, 2)
        fs = self.ajustar_fs(fs, FSV, 0.01)
    ...
    def evaluar_deslizamiento(self, parrilla: Parrilla, cargas: List[float], FSL: float) -> Tuple[float,float,bool]:
        """
        ...
        # Capacidad
        Q_L, memoria = parrilla.calculo_carga_lateral(F_zc)
