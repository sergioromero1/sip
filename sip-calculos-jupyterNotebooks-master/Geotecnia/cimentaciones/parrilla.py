...
class Parrilla:
    ...
    def __init__(self, B:float, 
        ...
        self.θ = θ
        self.peso = peso
        self.esf_act_ingedisa = esf_act_ingedisa
        self.mv_ingedisa = mv_ingedisa
        self.perfil = perfil
        self.α = α
        self.ω = ω
    ...
    def calculo_capacidad_portante_suelos_granulares(self, T:float, F_zc:float) -> Tuple[float, Memoria]:  
        ...
        """
        B, L, D, θ, perfil, α, ω  = self.get_atributos("B", "L", "D", "θ", "perfil", "α", "ω")
        ...
        # F_vc: Componente axial de la carga a compresión [kN]
        F_vc = F_zc + W_p
        ...
        ξ_γi = (1 - T / (F_zc + B_p * L_p * c_p * cot_g(φ)))**(n + 1)
        ξ_qi = (1 - T / (F_zc + B_p * L_p * c_p * cot_g(φ)))**(n)
    ...
    def calculo_tension_cono_tierra_suelos_granulares(self, q_adm:float) -> Tuple[float, Memoria]:
        ...
        Returns:
            Tuple[float, Memoria] -- (T_u: Capacidad a la tensión última [kN], memoria)
        """
        B, L, D, θ, perfil, γ_r  = self.get_atributos("B", "L", "D", "θ", "perfil", "γ_r")
        ...
        W_s = γ_r * (V_1) + 1 / 6 * γ * D**2 * tan_g(ψ) * (12 * B + 8 * D * tan_g(ψ))

        # T_u: Capacidad al arranque [kN]
        T_u = W_f + W_s

        # memoria
        memoria = {"tipo_mat": "g", "W_f":W_f, "V_1":V_1, "ψ":ψ, "γ":γ, "W_s":W_s}
