...
def main():
    ...
    for grupo in grupos:
        # if grupo["id"] == 9:
        datos = calcular(data_services, grupo, torres, info_cargas, parametros, usar_h_de_grupo, lista_HG, usar_tp_de_grupo)
        # data_services.actualizar_zapata_grupo(grupo)
        for dato in datos:
            print(grupo["id"],grupo["tipo"], grupo["concreto"], grupo["b"], grupo["d"], dato["torre"], dato["hg"], dato["caso_cargas"], dato["f_xc"], dato["f_yc"], dato["f_zc"], dato["q_min"], dato["q_max"])
            data_services.guardar_memoria_calculo_zapata_qs(grupo, dato)
    print("Done")

def calcular(data_services, grupo, torres, info_cargas, parametros, usar_h_de_grupo, lista_HG, usar_tp_de_grupo):
    B = grupo["b"]
    D = grupo["d"]
    ...
    Hs = []
    datos_memoria = []
    for nombre_torre in ["TCLL189"]: # grupo["torres"].split(","):
        ...
        for HG in lista_HG:
            ...
            if zapata:
                relleno = zapata.perfil.calcular_material_relleno(zapata.D - zapata.H)
                ...
                Q_max_d, Q_min_d, memoria = zapata.calculo_esfuerzo_actuante_sobre_suelo(F_xc, F_yc, F_zc)
                datos_memoria.append({"grupo_id": grupo["id"],"torre": nombre_torre, "hg": HG, "caso_cargas": "cargas_diseno_maxima_compresion", 
                                        "f_xc": F_xc, "f_yc": F_yc, "f_zc": F_zc, "q_max": Q_max_d, "q_min": Q_min_d, "memoria": memoria, "relleno": relleno})
                ...
                Q_max_t, Q_min_t, memoria = zapata.calculo_esfuerzo_actuante_sobre_suelo(F_xc, F_yc, F_zc)
                datos_memoria.append({"grupo_id": grupo["id"], "torre": nombre_torre, "hg": HG, "caso_cargas": "cargas_trabajo_maxima_compresion", 
                                        "f_xc": F_xc, "f_yc": F_yc, "f_zc": F_zc, "q_max": Q_max_t, "q_min": Q_min_t, "memoria": memoria, "relleno": relleno})
                ...
                Q_max_d, Q_min_d, memoria = zapata.calculo_esfuerzo_actuante_sobre_suelo(F_xc, F_yc, F_zc)
                datos_memoria.append({"grupo_id": grupo["id"], "torre": nombre_torre, "hg": HG, "caso_cargas": "cargas_diseno_maxima_longitudinal", 
                                        "f_xc": F_xc, "f_yc": F_yc, "f_zc": F_zc, "q_max": Q_max_d, "q_min": Q_min_d, "memoria": memoria, "relleno": relleno})
                ...
                Q_max_t, Q_min_t, memoria = zapata.calculo_esfuerzo_actuante_sobre_suelo(F_xc, F_yc, F_zc)
                datos_memoria.append({"grupo_id": grupo["id"], "torre": nombre_torre, "hg": HG, "caso_cargas": "cargas_trabajo_maxima_longitudinal", 
                                        "f_xc": F_xc, "f_yc": F_yc, "f_zc": F_zc, "q_max": Q_max_t, "q_min": Q_min_t, "memoria": memoria, "relleno": relleno})
                ...
                Q_max_d, Q_min_d, memoria = zapata.calculo_esfuerzo_actuante_sobre_suelo(F_xc, F_yc, F_zc)
                datos_memoria.append({"grupo_id": grupo["id"], "torre": nombre_torre, "hg": HG, "caso_cargas": "cargas_diseno_maxima_transversal", 
                                        "f_xc": F_xc, "f_yc": F_yc, "f_zc": F_zc, "q_max": Q_max_d, "q_min": Q_min_d, "memoria": memoria, "relleno": relleno})
                ...
                Q_max_t, Q_min_t, memoria = zapata.calculo_esfuerzo_actuante_sobre_suelo(F_xc, F_yc, F_zc)
                datos_memoria.append({"grupo_id": grupo["id"], "torre": nombre_torre, "hg": HG, "caso_cargas": "cargas_trabajo_maxima_transversal", 
                                        "f_xc": F_xc, "f_yc": F_yc, "f_zc": F_zc, "q_max": Q_max_t, "q_min": Q_min_t, "memoria": memoria, "relleno": relleno})
                ...
                # F_zc, F_xc, F_yc = info_cargas.obtener_cargas_trabajo_maxima_compresion_cartesiano(torre)
                # q_ult, memoria = zapata.calculo_capacidad_portante(max(F_xc, F_yc), F_zc)
